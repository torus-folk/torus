...
// torus
    // torus: money supply related block index fields
    int64_t nMint;
    int64_t nMoneySupply;

    // torus: proof-of-stake related block index fields
    unsigned int nFlags;  // torus: block index flags
    enum
    {
...
// torus end
    ...
    std::string ToString() const
    {
        return strprintf("CBlockIndex(nprev=%08x, nFile=%d, nHeight=%d, nMint=%s, nMoneySupply=%s, nFlags=(%s), bnStakeModifier=%s, hashProofOfStake=%s, prevoutStake=(%s), nStakeTime=%d merkle=%s, hashBlock=%s)",
            pprev, nFile, nHeight,
            FormatMoney(nMint), FormatMoney(nMoneySupply), IsProofOfStake()? "PoS" : "PoW", bnStakeModifier.ToString(),
