...
// torus: optional setting to unlock wallet for block minting only;
...
//    return n ? n : nTimeReceived;
    ...
    // torus: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// torus: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue;  // torus: timestamp must not exceed spend time
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    txNew.nTime = wtxNew.tx->nTime;  // torus: set time
...
//                }
                ...
                // torus: sub-cent change is moved to fee
...
// torus: create coin stake transaction
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew)
{
    ...
    int64_t nCombineThreshold = GetProofOfWorkReward(GetLastBlockIndex(chainActive.Tip(), false)->nHeight) / 3;
    ...
    if (gArgs.IsArgSet("-reservebalance") && !ParseMoney(gArgs.GetArg("-reservebalance", ""), nReserveBalance))
        return error("CreateCoinStake : invalid reserve balance amount");
    if (nBalance <= nReserveBalance) {
        LogPrintf("not enough nBalance %d nReserveBalance %d\n", nBalance, nReserveBalance);
        return false;
    }
    std::set<std::pair<const CWalletTx*,unsigned int> > setCoins;
    std::vector<const CWalletTx*> vwtxPrev;
    ...
    if (!SelectCoinsForStaking(txNew.nTime, nBalance - nReserveBalance, setCoins, nValueIn))
        return false;
    if (setCoins.empty()) {
        LogPrintf("-- CreateCoinStake: Have no coin to stake\n");
        return false;
    }
    CAmount nCredit = 0;
    ...
    for (; it != setCoins.end(); ++it) {
        ...
        for (unsigned int n=0; n < std::min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound; n++)
        {
            ...
            if (CheckKernel(nBits, pindexPrev, header, tx, prevoutStake, txNew.nTime - n))
            {
                // Found a kernel
