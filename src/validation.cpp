...
#if defined(NDEBUG)
# error "Torus cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Torus Signed Message:\n";
                              ...
                              bool bypass_limits, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // torus: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
...
int64_t GetProofOfWorkReward(unsigned int nHeight) {
    ...
    if (nHeight >= params.firstSuperBlock && nHeight <= params.lastSuperBlock) {
        ...
        if (epoch > 0 && epoch < 4) {
            ...
            return 0 * COIN; // POW ends after 2 years
        }
    }
}

// torus: miner's coin stake is rewarded based on coin age spent (coin-days)
int64_t GetProofOfStakeReward(int64_t nCoinAge)
{
    static int64_t nRewardCoinYear = 5 * CENT;  // creation amount per coin-year
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // torus: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // torus
            undo.nTime = alternate.nTime;           // torus
        } else {
...
bool TorusContextualBlockChecks(const CBlock &block, CValidationState &state, CBlockIndex *pindex, bool fJustCheck) {
    uint256 hashProofOfStake = uint256();
    uint256 targetProofOfStake = uint256();
    // torus: verify hash target and signature of coinstake tx
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!TorusContextualBlockChecks(block, state, pindex, fJustCheck))
        return error("%s: failed PoS check %s", __func__, FormatStateMessage(state));
    if (pindex->nHeight > params.nLastPOWBlock && block.IsProofOfWork())
    ...
    // torus: coinbase reward check relocated to CheckBlock()
    ...
    // torus: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // torus: fees are not collected by miners as in bitcoin
    // torus: fees are destroyed to compensate the entire network
    if (gArgs.GetBoolArg("-printcreation", false))
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature) {
    ...
    // torus: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // torus: first coinbase output should be empty if proof-of-stake block
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        // torus: check transaction timestamp
...
#ifdef ENABLE_CHECKPOINTS
    // torus: check that the block satisfies synchronized checkpoint
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    if (block.vtx[0]->GetValueOut() > (block.IsProofOfWork()? (GetProofOfWorkReward(nHeight) - nCoinbaseCost) : 0))
        return state.DoS(50, false, REJECT_INVALID, "bad-cb-amount", false,
                         strprintf("CheckBlock() : coinbase reward exceeded %s > %s",
                                   ...
                                   FormatMoney(block.IsProofOfWork()? GetProofOfWorkReward(nHeight) : 0)));
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // torus: Don't reject in case of old clients. Change our assumption instead.
...
//    // torus: ask for pending sync-checkpoint if any
//    if (!IsInitialBlockDownload())
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // torus: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // torus: check PoS
    if (fCheckPoS && !TorusContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // torus: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // torus: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(const CChainParams &chainparams) {
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // torus: txindex is always enabled
...
// torus: total coin age spent in transaction, in the unit of coin-days.
