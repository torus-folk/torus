...
// torus: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr <CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn, bool fMineWitnessTx, CWallet *pwallet, bool *pfPoSCancel) {
    ...
    if (pblock->IsProofOfWork()) {
        ...
        CAmount blockValue = GetProofOfWorkReward(nHeight);
    ...
    // torus: if coinstake available add coinstake tx
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (nSearchTime > nLastCoinStakeSearchTime)
        {
            ...
            if (pwallet->CreateCoinStake(*pwallet, pblock->nBits, nSearchInterval, txCoinStake))
            {
            ...
            nLastCoinStakeSearchInterval = nSearchInterval;
            nLastCoinStakeSearchTime = nSearchTime;
        }
        if (*pfPoSCancel)
            return nullptr; // torus: there is no point to continue if we failed to create coinstake
    }
...
//        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TorusMiner: generated block is stale");
    }
...
void PoSMiner(CWallet *pwallet) {
    ...
    RenameThread("torus-stake-minter");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                ...
                LogPrintf("Error in TorusMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            // torus: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake()) {
...
void MintStake(boost::thread_group &threadGroup) {
    // torus: mint proof-of-stake blocks in the background
    if (!vpwallets.empty())
...
void static TorusMiner(const CChainParams &chainparams, void *parg) {
    LogPrintf("TorusMiner started\n");
    RenameThread("torus-miner");
    CWallet *pwallet = (CWallet *) parg;
    ...
    try {
        ...
        while (true) {
            ...
            LogPrintf("TorusMiner: try create POW block\n");
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in TorusMiner: Keypool ran out, please call keypoolrefill before restarting the mining hread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running TorusMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        LOCK2(cs_main, pwallet->cs_wallet);
        ...
        LogPrintf("TorusMiner terminated: %s\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("TorusMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateTorus(bool fGenerate, int nThreads, const CChainParams &chainparams) {
    ...
    if (!vpwallets.empty()) {
        for (int i = 0; i < nThreads; i++)
            minerThreads->create_thread(boost::bind(&TorusMiner, boost::cref(chainparams), vpwallets[0]));
    }
}
